      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-semibold text-slate-800">
            {est.transportType ?? t("createShipment.shipmentFallback")}
          </p>
            ...
            {est.total != null ? (
              <>
                <p className="text-base leading-tight font-bold whitespace-nowrap text-slate-900">
                  {est.total.toLocaleString(intlLocale(i18n.language))}
              ...
              <p className="text-[11px] text-muted-foreground">{t("createShipment.onRequest")}</p>
            )}
            {est.freightOnly && (
              <p className="mt-0.5 text-[10px] font-medium whitespace-nowrap text-amber-600">
                {t("createShipment.freightOnly")}
              </p>
            )}
          </div>
          {hasDetails && (
            ...
            <div className="flex flex-col gap-1.5 border-t border-slate-100 px-4 py-2.5 text-[11px]">
              {est.transitPoint && (
                <DetailRow
                  ...
                  label={t("createShipment.detailTransit")}
              ...
              {est.breakdown && (
                <DetailRow
                  ...
                  label={t("createShipment.detailCalc")}
                  value={est.breakdown}
                />
              )}
              {est.included && (
                <DetailRow
                  ...
                  label={t("createShipment.detailIncluded")}
                  value={est.included}
                />
              )}
              {est.excluded && (
                <DetailRow
                  ...
                  label={t("createShipment.detailExcluded")}
                  value={est.excluded}
                />
              )}
              {est.transitTime && (
                <DetailRow
                  ...
                  label={t("createShipment.detailTerm")}
              ...
              {est.conditions && (
                <DetailRow
                  icon={<Info className="h-3.5 w-3.5 text-slate-400" />}
                  label={t("createShipment.detailConditions")}
                  value={est.conditions}
                />
              )}
              {est.sourceName && (
                <p className="mt-0.5 text-[10px] text-muted-foreground italic">
                  {t("createShipment.source")}: {est.sourceName}
                </p>
              )}
            </div>
          </div>
